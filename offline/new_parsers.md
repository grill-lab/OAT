...
Now you can try running the pipeline with `docker compose run offline` and check if the URLs are retrieved successfully by viewing the log messages.

The rest of the pipeline components should not need any updates and will generate fresh `TaskGraphs` from the newly downloaded pages.

## 5. Updating the system indexes

In order for the online OAT system to use the newly created indexes generated by the pipeline, you will need to:

* Delete all files/folders inside `OAT/shared/file_system/downloads/functionaltiies/system_indexes`
* Copy the `search_idx` and `objects_idx` folders from `OAT/shared/file_system/offline/system_indexes` to `OAT/shared/file_system/downloads/functionalities/system_indexes`
* (re)start the OAT online system
